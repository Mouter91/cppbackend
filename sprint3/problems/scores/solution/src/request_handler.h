...
class ApiHandler : public BaseHandler {
 public:
  explicit ApiHandler(model::Game& game, Strand& strand, extra_data::MapsExtra& extra)
      : game_(game), strand_(strand), maps_extra_(extra) {
  }
...
class RequestHandler {
 public:
  explicit RequestHandler(model::Game& game, Strand& strand,
                          const std::filesystem::path& static_root, extra_data::MapsExtra& extra)
      : api_handler_(game, strand, extra), static_handler_(static_root) {
  }
