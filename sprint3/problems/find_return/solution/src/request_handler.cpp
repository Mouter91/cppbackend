...
StringResponse ApiHandler::HandleGetMapById(const StringRequest& req,
  ...
  map_json["lootTypes"] = maps_extra_.GetLootTypes(map_id);
...
StringResponse ApiHandler::HandleGetGameState(const StringRequest& req) {
  return ExecuteAuthorized(req, [this, &req](const model::Player& player) {
    ...
    for (const auto& [dog_id, dog] : game_session->GetDogs()) {
      ...
      // Создаем массив для содержимого рюкзака
      json::array bag_json;
      const auto& bag_items = dog->GetBag().GetItems();
      for (size_t i = 0; i < bag_items.size(); ++i) {
        bag_json.push_back(json::object{
            {"id", static_cast<int>(i)},              // Идентификатор предмета в рюкзаке
            {"type", static_cast<int>(bag_items[i])}  // Тип предмета
        });
      }
      ...
      players_json[std::to_string(dog_id)] = {
          ...
          {"dir", direction},
          {"bag", bag_json}  // Добавляем информацию о рюкзаке
      };
    }
...
StringResponse ApiHandler::HandleGameTick(const StringRequest& req) {
  if (game_.GetSettings().IsAutoTickEnabled()) {
  ...
  try {
    ...
    double delta_time_sec = static_cast<double>(time_delta_ms) / 1000.0;
    game_.Tick(delta_time_sec);
