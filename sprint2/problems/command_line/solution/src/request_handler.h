...
public:
    explicit ApiHandler(model::Game& game, Strand& strand) 
        : game_(game), strand_(strand) {}

    template <typename Body, typename Allocator, typename Send>
    void HandleRequest(http::request<Body, http::basic_fields<Allocator>>&& req, Send&& send) {
        auto handle = [this, req = std::move(req), send = std::forward<Send>(send)]() mutable {
            static const std::unordered_map<std::string_view, std::vector<std::pair<http::verb, StringResponse(ApiHandler::*)(const StringRequest&)>>> handlers = {
                {"/api/v1/game/join", {{http::verb::post, &ApiHandler::HandleJoinGame}}},
                {"/api/v1/game/players", {{http::verb::get, &ApiHandler::HandleGetPlayers}, {http::verb::head, &ApiHandler::HandleGetPlayers}}},
                {"/api/v1/maps", {{http::verb::get, &ApiHandler::HandleGetMaps}, {http::verb::head, &ApiHandler::HandleGetMaps}}},
                {"/api/v1/game/state", {{http::verb::get, &ApiHandler::HandleGetGameState}, {http::verb::head, &ApiHandler::HandleGetGameState}}},
            ...
            if (it != handlers.end()) {
                ...
                return send(MakeMethodNotAllowedError({boost::beast::http::to_string(it->second.front().first)}));
            }
...
public:
    explicit RequestHandler(model::Game& game, Strand& strand, const std::filesystem::path& static_root)
        : api_handler_(game, strand), static_handler_(static_root) {}
