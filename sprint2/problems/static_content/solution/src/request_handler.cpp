...
StringResponse RequestHandler::createErrorResponse(http::status status, std::string_view code, std::string_view message) {
    // Определяем Content-Type в зависимости от типа ошибки
    std::string_view content_type = ContentType::APP_JSON;
    if (status == http::status::not_found && !code.starts_with("map")) {
        content_type = ContentType::TEXT_PLAIN;
    }
    if (status == http::status::bad_request && !code.starts_with("badRequest")) {
        content_type = ContentType::TEXT_PLAIN;
    }

    if (content_type == ContentType::APP_JSON) {
        json::value error = {{"code", code}, {"message", message}};
        return MakeStringResponse(status, json::serialize(error), 11, false, content_type);
    }
    return MakeStringResponse(status, message, 11, false, content_type);
}
}  // namespace http_handler
